...
render_pass::render_pass(vk::device* device, bool off_screen_rendering, size_t swap_chain_images, glm::vec2 dimensions)
{
    _off_screen_rendering = off_screen_rendering;
    _vk_frame_buffers.resize(swap_chain_images);
    _dimensions = dimensions;
    _device = device;
}

void render_pass::add_rendering_attachment(std::vector<texture_2d*>& rendering_textures)
{
    //note: the shader needs to expect the attachments set up in the
    //order in which they are attached (in order in which this function is called)
    
    _attachments.push_back(rendering_textures);
}
void render_pass::init()
{
    assert(_attachments.size() != 0 && "you need to attach something to render to");
    assert(_attachments.size() <= MAX_NUMBER_OF_ATTACHMENTS);
    
    uint32_t width = _attachments[0][0]->get_width();
    uint32_t height = _attachments[0][0]->get_height();
    ...
    for( uint32_t swapchain_index = 0; swapchain_index < _attachments.size(); ++swapchain_index)
    {
        ...
        {
            assert(width == _attachments[swapchain_index][attachment_index]->get_width());
            assert(height == _attachments[swapchain_index][attachment_index]->get_height());
            ...
            attachment_descriptions[attachment_index].format = static_cast<VkFormat>(_attachments[swapchain_index][attachment_index]->get_format());
        ...
        if(_depth_textures.size())
        {
            ...
            attachment_descriptions[_attachments[swapchain_index].size()-1] =  _depth_textures[swapchain_index]->get_depth_attachment();
...
{
    _vk_frame_buffers.resize(_vk_render_passes.size());
    ...
    for( size_t swapchain_index = 0; swapchain_index < _vk_frame_buffers.size(); ++swapchain_index)
    {
        for( size_t i = 0; i < _vk_frame_buffers.size(); ++i)
        {
            ...
            assert(_attachments[i].size() < MAX_NUMBER_OF_ATTACHMENTS);
            for( size_t j = 0; j < _attachments[i].size()-2; ++j)
            {
                attachment_views[j] = _attachments[i][j]->_image_view;
            }
            if(_depth_textures.size() != 0)
            {
                assert(_depth_textures.size() == _attachments.size());
                attachment_views[_attachments[i].size()-1]  = _depth_textures[swapchain_index]->_image_view;
            }
            ...
            framebuffer_create_info.width = _attachments[0][0]->get_width();
            framebuffer_create_info.height = _attachments[0][0]->get_height();
