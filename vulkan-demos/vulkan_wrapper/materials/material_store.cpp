...
mat_shared_ptr material_store::get_material(const char* name) const
{
    ...
    if(material_database[name]->get_in_use())
    {
       ...
       {
           tmp = CREATE_MAT<visual_material>( static_cast<visual_material&> (*material_database[name]) );
       }
       else
       {
           tmp = CREATE_MAT<compute_material>( static_cast<compute_material&> (*material_database[name]) );
       }
    }
    tmp->set_in_use();
    return tmp;
