...
VkBool32 demo_check_layers(const std::vector<layer_properties> &layer_props, const std::vector<const char *> &layer_names) {
    uint32_t check_count = static_cast<uint32_t>(layer_names.size());
    uint32_t layer_count = static_cast<uint32_t>(layer_props.size());
...
VkResult init_instance(struct sample_info &info, char const *const app_short_name) {
    ...
    inst_info.ppEnabledLayerNames =  (uint32_t)info.instance_layer_names.size() ? info.instance_layer_names.data() : NULL;
